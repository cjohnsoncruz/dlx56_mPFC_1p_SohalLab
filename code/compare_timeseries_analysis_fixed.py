...
FIXED: Handles neuron_id data type mismatch
"""
...
print(f"  Data type: {canon_df[neuron_col_canon].dtype}")
print(f"  Sample values: {canon_df[neuron_col_canon].head(3).tolist()}")

print(f"\nNeuron ID column in NEW: '{neuron_col_new}'")
print(f"  Data type: {new_df[neuron_col_new].dtype}")
print(f"  Sample values: {new_df[neuron_col_new].head(3).tolist()}")
...
# Check overlap - convert to string for comparison
canon_neurons = set(canon_df[neuron_col_canon].astype(str).unique())
new_neurons = set(new_df[neuron_col_new].astype(str).unique())
...
if len(canon_neurons - new_neurons) > 0 and len(canon_neurons - new_neurons) <= 10:
    print(f"  {sorted(canon_neurons - new_neurons)}")
print(f"Only in NEW: {len(new_neurons - canon_neurons)}")
if len(new_neurons - canon_neurons) > 0 and len(new_neurons - canon_neurons) <= 10:
    print(f"  {sorted(new_neurons - canon_neurons)}")
...
# Rename columns for merging and CONVERT TO STRING to match data types
...
canon_mean_renamed['neuron_id'] = canon_mean_renamed['neuron_id'].astype(str)
...
new_mean_renamed['neuron_id'] = new_mean_renamed['neuron_id'].astype(str)

print(f"CANON neuron_id dtype after conversion: {canon_mean_renamed['neuron_id'].dtype}")
print(f"NEW neuron_id dtype after conversion: {new_mean_renamed['neuron_id'].dtype}")
...
print("="*80)
print(f"\nSUMMARY:")
print(f"  Total neurons in CANON: {len(canon_neurons)}")
print(f"  Total neurons in NEW: {len(new_neurons)}")
print(f"  Neurons in both: {len(canon_neurons & new_neurons)}")
